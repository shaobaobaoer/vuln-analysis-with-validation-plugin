...
> **Full validation flow**: `skills/_shared/validation_framework.md` (authoritative source)
...
- `skills/_shared/validation_framework.md` for validation flow

## Execution Flow
1. **Pre-flight check**: Verify container running + app responds. Entry point verification per vulnerability.
2. **Environment init**: Deploy trigger binary, start TCP listeners, set up inotifywait (see `validation_framework.md §Step 1`)
3. **Execute PoCs**: `docker exec <container> python3 /app/poc_scripts/<script> --target http://localhost:<port>`
4. **Legitimacy check**: Scan PoC source for forbidden patterns (see `validation_framework.md §Step 3`)
5. **Type-specific verification**: Check success conditions (see `validation_framework.md §Step 4`)
